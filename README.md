...
Для работы с удаленными репозиториями мы будем использовать ресурс GitHub. Он позволяет управлять/делиться своими/чужими репозиториями.

## Вариантзагрузки своего (локально расположенного) в свой профиль GitHub.

 Если профиля нет, то необходимо его создать.

 1. В верхней правой части экрана необходимо нажать знак "+" -> New repository -> Ввести имя репозитория -> нажать кнопку Create repository. При этом режим доступа к ней будет Public

 2. Кликнув на профиль в верхней правой части экрана перейти в созданный репозиторий 

 3. Cкопировать ссылку на созданный репозиторий.

 4. На странице, которая открывается после создания предложены 2 варианта для "связывания" созданного репозитория и вашего локального. Выберете подходящий Вам вариант.

 5. Скопируйте и вставьте выбранный код в терминал.

**Готово**

Применяемы команды:
 - git push - Для выгрузки локальных изменений на GitHub (после add и commit)
 - git pull - Для загрузки изменений, внесенных удаленно в локальный репозиторий


## Вариант выгрузки чужого репозитория для работы

Для этого нам потребуется найти интересующий нас репозиторий GitHub.

1. В верхней части экрана нажать кнопку Fork для добавления копии в свой репозиторий GitHub

2. Зайти в свой профиль, кликнуть на добавленный репозиторий.

3. Нажать кнопку Code и скопировать ссылку.

4. Открыть необходиму папку (где будет храниться и обрабатываться репозиторий) локально.

5. В терминале прописать команду 
```
git clone <ссылка на репозиторий GitHub>
```

6. Готово! Работайте с репозиторием локально.

Всю работу при таком варианте принято осуществлять в отдельной ветке. Для этого используйте ранее изученную команду 
```
git branch <имя новой ветки>
```

Для загрузки изменений с свою копию клонированного репозитория воспользуйтесь командами
```
git add .
git commit -m " "
git push
...
GitHub сам предложит вам поделиться изменениями с автором изначального репозитория
