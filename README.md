...
Для начала работы с удаленными репозиториями необходимо пройти регистрацию на [GitHub](https://github.com/) и следовать инструкциям.

После этого в правом верхнем углу нажать на знак "+". В открывшемся меню выбрать *New repository*. После этого написать название репозитория и выбрать его тип(публичный или приватный). Далее снизу подтвердить создание репозитория кнопкой *Create repository*.
После этого выбрать каким образом будет создан файл в реопзитории из четырех предложенных вариантов. Выбрать первый вариант, задать название файла, написать что-либо в 1 строчке консоли и сделать снизу commit. => **Готово**, теперь мы имеем удаленный репозиторий.

### Выгрузка проекта из удаленного репозитория.

Для того, чтобы выгрузить проект необходимо перейти в раздел репозиториев и выбрать нужный репозиторий. Нажать на кнопку **Code** и скопировать приведенный адресный путь, например: https://github.com/Test/Simple.git
После этого перейти в VisualStudioCode, создать новую папку, в которую поместим скачанный проект, открыть терминал ввести команду "git clone" и добавить скопированную ссылку. Теперь можно работать в проекте локально!

### Загрузка локального проекта в удаленный репозиторий.

Для того, чтобы загрузить проект в GitHub необходимо следовать инструкциям создания нового репозитория, но при этом, при его создании необходимо выбрать третий пункт под названием *…or push an existing repository from the command line*. Копировать указанные в этом пункте команды и вставить их в терминал VisualStudioCode. После этого проект добавится автоматически.
